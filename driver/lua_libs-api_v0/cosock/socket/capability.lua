...
setmetatable(m, {__call = function()
  local inner_sock, err = lsocket.capability()
  if not inner_sock then return inner_sock, err end
  inner_sock:settimeout(0)
  return setmetatable({inner_sock = inner_sock, class = "capability{}"}, { __index = m})
end})
...
m.get_capability_definition = passthrough("get_capability_definition")
