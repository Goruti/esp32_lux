...
function device_management.build_bind_request(device, cluster, hub_zigbee_eui)
  ...
  local bind_req = bind_request.BindRequest(device.zigbee_eui, device:get_endpoint(cluster), cluster, bind_request.ADDRESS_MODE_64_BIT, hub_zigbee_eui, constants.HUB.ENDPOINT)
...
--- @param attr_config AttributeConfiguration the attribute configuration to set up
...
function device_management.configure_ias_zone(device, ias_zone_configuration_method, cie_address)
  ...
  elseif ias_zone_configuration_method == constants.IAS_ZONE_CONFIGURE_TYPE.CUSTOM then
    log.info("IAS Zone configuration method set to custom, not automatically configuring.")
  end
end
...
function device_management.configure(driver, device)
  if driver.environment_info.hub_zigbee_eui == nil then
    log.warn("Can't configure Zigbee device without hub Zigbee EUI information")
    return
  end
