...
function ZigbeeMessageAttributeHandler.init(cls, cluster, attribute, handlers)
  ...
  -- Create a bound handler to convert the `attr` handler format to one that has the
  -- same argument structure as all other handlers so they can be included in a list
  -- alongside the others.
  for _, handler in ipairs(handler_list) do
    local bound_attr_handler = function(driver, device, zb_rx)
      handler(driver, device, zb_rx.body.zcl_body:get_attribute_data(attribute), zb_rx)
    end
    table.insert(bound_handlers, bound_attr_handler)
  end
  local out = ZigbeeMessageHandler.init(cls, "ZclClusterAttributeValueHandler", bound_handlers)
...
function ZigbeeMessageDispatcher:find_default_handlers(driver, device, zb_rx)
  ...
  if zb_rx ~= nil then
    if zb_rx.address_header.profile.value == constants.HA_PROFILE_ID then
      ...
      if handler_set ~= "cluster" and (zb_rx.body.zcl_header.cmd.value == zcl_commands.READ_ATTRIBUTE_RESPONSE_ID or zb_rx.body.zcl_header.cmd.value == zcl_commands.REPORT_ATTRIBUTE_ID) then
        ...
        for _, v in ipairs(zb_rx.body.zcl_body.attr_records) do
          if (v.status == nil or v.status.value == Status.SUCCESS) and (self.default_handlers.attr[zb_rx.address_header.cluster.value] or {})[v.attr_id.value] ~= nil then
            add_handlers_to_list(self.default_handlers.attr[zb_rx.address_header.cluster.value][v.attr_id.value], matching_handlers)
          end
        end
      end
