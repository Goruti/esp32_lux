...
function ResetAllAlarms.deserialize(buf)
  local out = {}
  for _, v in ipairs(ResetAllAlarms.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing ResetAllAlarms")
    end
  end
  setmetatable(out, {__index = ResetAllAlarms})
  out:set_field_names()
  return out
end
...
function ResetAllAlarms.build_test_rx(device)
  local out = {}
  local args = {}
  for i,v in ipairs(ResetAllAlarms.args_def) do
  ...
  setmetatable(out, {__index = ResetAllAlarms})
  out:set_field_names()
  return ResetAllAlarms._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end
...
function ResetAllAlarms:init(device)
  ...
  setmetatable(out, {
    __index = ResetAllAlarms,
    __tostring = ResetAllAlarms.pretty_print
  })
  out:set_field_names()
