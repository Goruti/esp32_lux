...
function StepSaturation.deserialize(buf)
  local out = {}
  for _, v in ipairs(StepSaturation.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing StepSaturation")
    end
  end
  setmetatable(out, {__index = StepSaturation})
  out:set_field_names()
  return out
end
...
function StepSaturation.build_test_rx(device, step_mode, step_size, transition_time, options_mask, options_override)
  ...
  for i,v in ipairs(StepSaturation.args_def) do
  ...
  setmetatable(out, {__index = StepSaturation})
  out:set_field_names()
  return StepSaturation._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end
...
function StepSaturation:init(device, step_mode, step_size, transition_time, options_mask, options_override)
  ...
  setmetatable(out, {
    __index = StepSaturation,
    __tostring = StepSaturation.pretty_print
  })
  out:set_field_names()
