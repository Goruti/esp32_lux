...
function MoveWithOnOff.deserialize(buf)
  local out = {}
  for _, v in ipairs(MoveWithOnOff.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing MoveWithOnOff")
    end
  end
  setmetatable(out, {__index = MoveWithOnOff})
  out:set_field_names()
  return out
end
...
function MoveWithOnOff.build_test_rx(device, move_mode, rate, options_mask, options_override)
  ...
  for i,v in ipairs(MoveWithOnOff.args_def) do
  ...
  setmetatable(out, {__index = MoveWithOnOff})
  out:set_field_names()
  return MoveWithOnOff._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end
...
function MoveWithOnOff:init(device, move_mode, rate, options_mask, options_override)
  ...
  setmetatable(out, {
    __index = MoveWithOnOff,
    __tostring = MoveWithOnOff.pretty_print
  })
  out:set_field_names()
