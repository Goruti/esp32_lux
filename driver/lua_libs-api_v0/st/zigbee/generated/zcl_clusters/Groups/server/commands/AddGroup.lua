...
--- @field public NAME string "AddGroup" the name of this command
...
AddGroup.get_length = utils.length_from_fields
AddGroup._serialize = utils.serialize_from_fields
AddGroup.pretty_print = utils.print_from_fields

--- Deserialize this command
---
--- @param buf buf the bytes of the command body
--- @return AddGroup
function AddGroup.deserialize(buf)
  local out = {}
  for _, v in ipairs(AddGroup.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing AddGroup")
    end
  end
  setmetatable(out, {__index = AddGroup})
  out:set_field_names()
  return out
end

function AddGroup:set_field_names()
  for _, v in ipairs(self.args_def) do
    if self[v.name] ~= nil then
...
function AddGroup.build_test_rx(device, group_id, group_name)
  local out = {}
  local args = {group_id, group_name}
  for i,v in ipairs(AddGroup.args_def) do
  ...
  setmetatable(out, {__index = AddGroup})
  out:set_field_names()
  return AddGroup._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end

--- Initialize the AddGroup command
---
--- @param self AddGroup the template class for this command
--- @param device st.zigbee.Device the device to build this message to
--- @param group_id st.zigbee.data_types.Uint16
--- @param group_name st.zigbee.data_types.CharString
--- @return st.zigbee.ZigbeeMessageTx the full command addressed to the device
function AddGroup:init(device, group_id, group_name)
  local out = {}
  local args = {group_id, group_name}
  ...
  setmetatable(out, {
    __index = AddGroup,
    __tostring = AddGroup.pretty_print
  })
  out:set_field_names()
  return self._cluster:build_cluster_specific_command(device, out, "server")
end

function AddGroup:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

setmetatable(AddGroup, {__call = AddGroup.init})
