...
function AddGroupResponse.deserialize(buf)
  local out = {}
  for _, v in ipairs(AddGroupResponse.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing AddGroupResponse")
    end
  end
  setmetatable(out, {__index = AddGroupResponse})
  out:set_field_names()
  return out
end
...
function AddGroupResponse.build_test_rx(device, status, group_id)
  ...
  for i,v in ipairs(AddGroupResponse.args_def) do
  ...
  setmetatable(out, {__index = AddGroupResponse})
  out:set_field_names()
  return AddGroupResponse._cluster:build_test_rx_cluster_specific_command(device, out, "client")
end
...
function AddGroupResponse:init(device, status, group_id)
  ...
  setmetatable(out, {
    __index = AddGroupResponse,
    __tostring = AddGroupResponse.pretty_print
  })
  out:set_field_names()
