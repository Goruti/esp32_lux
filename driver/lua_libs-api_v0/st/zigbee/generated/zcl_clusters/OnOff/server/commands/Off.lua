...
function Off.deserialize(buf)
  local out = {}
  for _, v in ipairs(Off.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing Off")
    end
  end
  setmetatable(out, {__index = Off})
  out:set_field_names()
  return out
end
...
function Off.build_test_rx(device)
  local out = {}
  local args = {}
  for i,v in ipairs(Off.args_def) do
  ...
  setmetatable(out, {__index = Off})
  out:set_field_names()
  return Off._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end
