...
function GetAlertsResponse.deserialize(buf)
  local out = {}
  for _, v in ipairs(GetAlertsResponse.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing GetAlertsResponse")
    end
  end
  setmetatable(out, {__index = GetAlertsResponse})
  out:set_field_names()
  return out
end
...
function GetAlertsResponse.build_test_rx(device, alerts_count, alert_structure)
  ...
  for i,v in ipairs(GetAlertsResponse.args_def) do
  ...
  setmetatable(out, {__index = GetAlertsResponse})
  out:set_field_names()
  return GetAlertsResponse._cluster:build_test_rx_cluster_specific_command(device, out, "client")
end
...
function GetAlertsResponse:init(device, alerts_count, alert_structure)
  ...
  setmetatable(out, {
    __index = GetAlertsResponse,
    __tostring = GetAlertsResponse.pretty_print
  })
  out:set_field_names()
