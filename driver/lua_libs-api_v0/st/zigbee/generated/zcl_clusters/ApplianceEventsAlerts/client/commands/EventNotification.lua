...
function EventNotification.deserialize(buf)
  local out = {}
  for _, v in ipairs(EventNotification.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing EventNotification")
    end
  end
  setmetatable(out, {__index = EventNotification})
  out:set_field_names()
  return out
end
...
function EventNotification.build_test_rx(device, event_header, event_id)
  ...
  for i,v in ipairs(EventNotification.args_def) do
  ...
  setmetatable(out, {__index = EventNotification})
  out:set_field_names()
  return EventNotification._cluster:build_test_rx_cluster_specific_command(device, out, "client")
end
...
function EventNotification:init(device, event_header, event_id)
  ...
  setmetatable(out, {
    __index = EventNotification,
    __tostring = EventNotification.pretty_print
  })
  out:set_field_names()
