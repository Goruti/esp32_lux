...
function FastPollStop.deserialize(buf)
  local out = {}
  for _, v in ipairs(FastPollStop.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing FastPollStop")
    end
  end
  setmetatable(out, {__index = FastPollStop})
  out:set_field_names()
  return out
end
...
function FastPollStop.build_test_rx(device)
  local out = {}
  local args = {}
  for i,v in ipairs(FastPollStop.args_def) do
  ...
  setmetatable(out, {__index = FastPollStop})
  out:set_field_names()
  return FastPollStop._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end
...
function FastPollStop:init(device)
  ...
  setmetatable(out, {
    __index = FastPollStop,
    __tostring = FastPollStop.pretty_print
  })
  out:set_field_names()
