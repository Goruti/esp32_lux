...
function SetShortPollInterval.deserialize(buf)
  local out = {}
  for _, v in ipairs(SetShortPollInterval.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing SetShortPollInterval")
    end
  end
  setmetatable(out, {__index = SetShortPollInterval})
  out:set_field_names()
  return out
end
...
function SetShortPollInterval.build_test_rx(device, new_short_poll_interval)
  ...
  for i,v in ipairs(SetShortPollInterval.args_def) do
  ...
  setmetatable(out, {__index = SetShortPollInterval})
  out:set_field_names()
  return SetShortPollInterval._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end
...
function SetShortPollInterval:init(device, new_short_poll_interval)
  ...
  setmetatable(out, {
    __index = SetShortPollInterval,
    __tostring = SetShortPollInterval.pretty_print
  })
  out:set_field_names()
