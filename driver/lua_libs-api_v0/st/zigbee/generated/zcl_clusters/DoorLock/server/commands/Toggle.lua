...
function Toggle.deserialize(buf)
  local out = {}
  for _, v in ipairs(Toggle.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing Toggle")
    end
  end
  setmetatable(out, {__index = Toggle})
  out:set_field_names()
  return out
end
...
function Toggle.build_test_rx(device, pin_or_rfid_code)
  ...
  setmetatable(out, {__index = Toggle})
  out:set_field_names()
  return Toggle._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end
