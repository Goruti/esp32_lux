...
function GetUserTypeResponse.deserialize(buf)
  local out = {}
  for _, v in ipairs(GetUserTypeResponse.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing GetUserTypeResponse")
    end
  end
  setmetatable(out, {__index = GetUserTypeResponse})
  out:set_field_names()
  return out
end
...
function GetUserTypeResponse.build_test_rx(device, user_id, user_type)
  ...
  for i,v in ipairs(GetUserTypeResponse.args_def) do
  ...
  setmetatable(out, {__index = GetUserTypeResponse})
  out:set_field_names()
  return GetUserTypeResponse._cluster:build_test_rx_cluster_specific_command(device, out, "client")
end
...
function GetUserTypeResponse:init(device, user_id, user_type)
  ...
  setmetatable(out, {
    __index = GetUserTypeResponse,
    __tostring = GetUserTypeResponse.pretty_print
  })
  out:set_field_names()
