...
function SetHolidayScheduleResponse.deserialize(buf)
  local out = {}
  for _, v in ipairs(SetHolidayScheduleResponse.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing SetHolidayScheduleResponse")
    end
  end
  setmetatable(out, {__index = SetHolidayScheduleResponse})
  out:set_field_names()
  return out
end
...
function SetHolidayScheduleResponse.build_test_rx(device, status)
  local out = {}
  local args = {status}
  for i,v in ipairs(SetHolidayScheduleResponse.args_def) do
  ...
  setmetatable(out, {__index = SetHolidayScheduleResponse})
  out:set_field_names()
  return SetHolidayScheduleResponse._cluster:build_test_rx_cluster_specific_command(device, out, "client")
end
...
function SetHolidayScheduleResponse:init(device, status)
  ...
  setmetatable(out, {
    __index = SetHolidayScheduleResponse,
    __tostring = SetHolidayScheduleResponse.pretty_print
  })
  out:set_field_names()
