...
function ClearRFIDCodeResponse.deserialize(buf)
  local out = {}
  for _, v in ipairs(ClearRFIDCodeResponse.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing ClearRFIDCodeResponse")
    end
  end
  setmetatable(out, {__index = ClearRFIDCodeResponse})
  out:set_field_names()
  return out
end
...
function ClearRFIDCodeResponse.build_test_rx(device, status)
  local out = {}
  local args = {status}
  for i,v in ipairs(ClearRFIDCodeResponse.args_def) do
  ...
  setmetatable(out, {__index = ClearRFIDCodeResponse})
  out:set_field_names()
  return ClearRFIDCodeResponse._cluster:build_test_rx_cluster_specific_command(device, out, "client")
end
...
function ClearRFIDCodeResponse:init(device, status)
  ...
  setmetatable(out, {
    __index = ClearRFIDCodeResponse,
    __tostring = ClearRFIDCodeResponse.pretty_print
  })
  out:set_field_names()
