...
function UpOrOpen.deserialize(buf)
  local out = {}
  for _, v in ipairs(UpOrOpen.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing UpOrOpen")
    end
  end
  setmetatable(out, {__index = UpOrOpen})
  out:set_field_names()
  return out
end
...
function UpOrOpen.build_test_rx(device)
  local out = {}
  local args = {}
  for i,v in ipairs(UpOrOpen.args_def) do
  ...
  setmetatable(out, {__index = UpOrOpen})
  out:set_field_names()
  return UpOrOpen._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end
...
function UpOrOpen:init(device)
  ...
  setmetatable(out, {
    __index = UpOrOpen,
    __tostring = UpOrOpen.pretty_print
  })
  out:set_field_names()
