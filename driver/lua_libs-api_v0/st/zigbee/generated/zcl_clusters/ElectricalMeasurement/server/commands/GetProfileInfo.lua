...
function GetProfileInfo.deserialize(buf)
  ...
  for _, v in ipairs(GetProfileInfo.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing GetProfileInfo")
    end
  end
...
function GetProfileInfo.build_test_rx(device)
  ...
  setmetatable(out, {__index = GetProfileInfo})
  out:set_field_names()
  return GetProfileInfo._cluster:build_test_rx_cluster_specific_command(device, out, "server")
end
