...
function EnhancedViewSceneResponse.deserialize(buf)
  local out = {}
  for _, v in ipairs(EnhancedViewSceneResponse.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing EnhancedViewSceneResponse")
    end
  end
  setmetatable(out, {__index = EnhancedViewSceneResponse})
  out:set_field_names()
  return out
end
...
function EnhancedViewSceneResponse.build_test_rx(device, status, group_id, scene_id, transition_time, scene_name, extension_field_sets)
  ...
  for i,v in ipairs(EnhancedViewSceneResponse.args_def) do
  ...
  setmetatable(out, {__index = EnhancedViewSceneResponse})
  out:set_field_names()
  return EnhancedViewSceneResponse._cluster:build_test_rx_cluster_specific_command(device, out, "client")
end
...
function EnhancedViewSceneResponse:init(device, status, group_id, scene_id, transition_time, scene_name, extension_field_sets)
  ...
  setmetatable(out, {
    __index = EnhancedViewSceneResponse,
    __tostring = EnhancedViewSceneResponse.pretty_print
  })
  out:set_field_names()
