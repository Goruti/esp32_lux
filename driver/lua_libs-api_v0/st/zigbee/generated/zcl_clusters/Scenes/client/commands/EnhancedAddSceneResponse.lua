...
function EnhancedAddSceneResponse.deserialize(buf)
  local out = {}
  for _, v in ipairs(EnhancedAddSceneResponse.args_def) do
    ...
    elseif not v.optional then
      log.debug("Missing command arg " .. v.name .. " for deserializing EnhancedAddSceneResponse")
    end
  end
  setmetatable(out, {__index = EnhancedAddSceneResponse})
  out:set_field_names()
  return out
end
...
function EnhancedAddSceneResponse.build_test_rx(device, status, group_id, scene_id)
  ...
  for i,v in ipairs(EnhancedAddSceneResponse.args_def) do
  ...
  setmetatable(out, {__index = EnhancedAddSceneResponse})
  out:set_field_names()
  return EnhancedAddSceneResponse._cluster:build_test_rx_cluster_specific_command(device, out, "client")
end
...
function EnhancedAddSceneResponse:init(device, status, group_id, scene_id)
  ...
  setmetatable(out, {
    __index = EnhancedAddSceneResponse,
    __tostring = EnhancedAddSceneResponse.pretty_print
  })
  out:set_field_names()
