...
--- @field public NAME string "CurrentGroup" the name of this attribute
--- @field public data_type st.zigbee.zcl.clusters.Scenes.types.SGroupId the data type of this attribute
local CurrentGroup = {
  ID = 0x0002,
  NAME = "CurrentGroup",
  base_type = SGroupIdType,
}

function CurrentGroup:augment_type(base_type_obj)
...
function CurrentGroup.pretty_print(value_obj)
  CurrentGroup.base_type.pretty_print(value_obj)
end

--- @function CurrentGroup:build_test_attr_report
---
--- Build a Rx Zigbee message as if a device reported this attribute
--- @param device st.zigbee.Device
--- @param data st.zigbee.zcl.clusters.Scenes.types.SGroupId the attribute value
--- @return st.zigbee.ZigbeeMessageRx containing an AttributeReport body
CurrentGroup.build_test_attr_report = cluster_base.build_test_attr_report

--- @function CurrentGroup:build_test_read_attr_response
---
--- Build a Rx Zigbee message as if a device sent a read response for this attribute
--- @param device st.zigbee.Device
--- @param data st.zigbee.zcl.clusters.Scenes.types.SGroupId the attribute value
--- @return st.zigbee.ZigbeeMessageRx containing an ReadAttributeResponse body
CurrentGroup.build_test_read_attr_response = cluster_base.build_test_read_attr_response
...
--- @return st.zigbee.zcl.clusters.Scenes.types.SGroupId
function CurrentGroup:new_value(...)
...
function CurrentGroup:read(device)
...
--- @param rep_change st.zigbee.zcl.clusters.Scenes.types.SGroupId The amount of change of the attribute to trigger a report
--- @return st.zigbee.ZigbeeMessageTx containing a ConfigureReporting body
function CurrentGroup:configure_reporting(device, min_rep_int, max_rep_int, rep_change)
...
function CurrentGroup:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

setmetatable(CurrentGroup, {__call = CurrentGroup.new_value})
return CurrentGroup
