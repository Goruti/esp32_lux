...
function WriteAttributeResponseRecord.init(cls, status, attr_id)
  local out = {}
  out.status = data_types.validate_or_build_type(status, ZclStatus, "status")
...
function WriteAttributeResponse.init(cls, global_status, write_attr_resp_records)
  ...
  if global_status ~= nil then
    out.global_status = data_types.validate_or_build_type(global_status, ZclStatus, "global_status")
  else
