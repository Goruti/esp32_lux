...
function data_types.validate_or_build_type(value, data_type, name)
  if type(value) == "table" and value.ID ~= data_type.ID then
    error(string.format("Expecting %s (0x%02X) for \"%s\" received %s", data_type.NAME, data_type.ID, name, tostring(value)), 2)
  elseif type(value) == "table" then
    ...
    if not status then
      error( "Error creating " .. name .. ": " .. tostring(out_val))
    end
