...
function lock_defaults.lock_operation_event_handler(driver, device, zb_rx)
  ...
  if (event ~= nil) then
    ...
    if (source == 0) then
      ...
      event.data = {method = METHOD[0], codeId = code_id .. "", usedCode = code_id, codeName = code_name}
    end
...
lock_defaults.capability_handlers = {
  [capabilities.lock.commands.lock] = lock_defaults.lock,
  [capabilities.lock.commands.unlock] = lock_defaults.unlock,
}
