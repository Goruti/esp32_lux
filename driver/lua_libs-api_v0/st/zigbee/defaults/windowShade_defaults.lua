...
windowShade_defaults.SHADE_LEVEL_KEY = "_shadeLevel"
...
--- @param ZigbeeDevice st.zigbee.Device The device this message was received from containing identifying information
...
function windowShade_defaults.default_current_lift_percentage_handler(ZigbeeDriver, ZigbeeDevice, value, zb_rx)
  local last_level = ZigbeeDevice:get_field(windowShade_defaults.SHADE_LEVEL_KEY)
  local windowShade = capabilities.windowShade.windowShade
  local event = nil
  local current_level = value.value
  if current_level ~= last_level or last_level == nil then
    ZigbeeDevice:set_field(windowShade_defaults.SHADE_LEVEL_KEY, current_level, {persist = true})
    last_level = last_level and last_level or 0
    ZigbeeDevice:emit_event_for_endpoint(zb_rx.address_header.src_endpoint.value, capabilities.windowShadeLevel.shadeLevel(current_level))
  ...
  if event ~= nil then
    ZigbeeDevice:emit_event_for_endpoint(zb_rx.address_header.src_endpoint.value, event)
    ZigbeeDevice.thread:call_with_delay(1, function(d)
      local current_level = ZigbeeDevice:get_field(windowShade_defaults.SHADE_LEVEL_KEY)
      if current_level > 0 and current_level < 100 then
        ZigbeeDevice:emit_event_for_endpoint(zb_rx.address_header.src_endpoint.value, windowShade.partially_open())
      end
    end
    )
  end
...
local function build_window_shade_cmd(cmd_type)
  return function(ZigbeeDriver, ZigbeeDevice, command)
    ZigbeeDevice:send_to_component(command.component, cmd_type(ZigbeeDevice))
  end
end
...
windowShade_defaults.capability_handlers = {
  [capabilities.windowShade.commands.open] = build_window_shade_cmd(zcl_clusters.WindowCovering.server.commands.UpOrOpen),
  [capabilities.windowShade.commands.close] = build_window_shade_cmd(zcl_clusters.WindowCovering.server.commands.DownOrClose),
  [capabilities.windowShade.commands.pause] = build_window_shade_cmd(zcl_clusters.WindowCovering.server.commands.Stop)
}
