...
--- @field public attr number the attribute ID
...
--- @field public rep_change st.zigbee.data_types.DataType (optional) the amount of change needed to trigger a report.  Only necessary for non-discrete attributes
...
--- @field public cluster_configurations AttributeConfiguration[] A list of configurations for reporting attributes
...
function ZigbeeDriver:zigbee_message_handler(message_channel)
  ...
  local zb_rx = messages.ZigbeeMessageRx.deserialize(buf)
...
function ZigbeeDriver.init(cls, name, driver_template)
  ...
  out_driver:register_channel_handler(out_driver.zigbee_channel, out_driver.zigbee_message_handler, "zigbee")
  ZigbeeDriver.populate_zigbee_dispatcher_from_sub_drivers(out_driver)
  log.trace(string.format("Setup driver %s with Zigbee handlers:\n%s", out_driver.NAME, out_driver.zigbee_message_dispatcher))
  ------------------------------------------------------------------------------------
  -- Set up local state
  ------------------------------------------------------------------------------------
