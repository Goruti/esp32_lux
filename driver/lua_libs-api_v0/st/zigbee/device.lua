...
function ZigbeeDevice:supports_server_cluster(cluster_id)
  local found_cluster = false
  for ep_id, ep in pairs(self.zigbee_endpoints) do
...
function ZigbeeDevice:add_configured_attribute(config)
  if not self:supports_server_cluster(config.cluster) then
    log.warn(string.format("Device does not support cluster 0x%04X not adding configured attribute", config.cluster))
    return
  end
...
function ZigbeeDevice:add_monitored_attribute(config)
  if not self:supports_server_cluster(config.cluster) then
    log.warn(string.format("Device does not support cluster 0x%04X not adding monitored attribute", config.cluster))
    return
  end
...
function ZigbeeDevice:send(zb_tx)
  if type(zb_tx) == "table" and getmetatable(zb_tx) == zigbee_messages.ZigbeeMessageTx then
    self.log.info(string.format("sending Zigbee message: %s", zb_tx))
