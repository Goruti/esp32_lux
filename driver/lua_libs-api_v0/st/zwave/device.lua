...
function ZwaveDevice:set_component_to_endpoint_fn(comp_ep_fn)
  if self:get_field(COMPONENT_TO_ENDPOINT_FUNC) == nil then
    self:set_field(COMPONENT_TO_ENDPOINT_FUNC, comp_ep_fn)
  else
    log.error("Attempt to re-set valid component-to-endpoint function.")
  end
end
...
function ZwaveDevice:set_endpoint_to_component_fn(ep_comp_fn)
  if self:get_field(ENDPOINT_TO_COMPONENT_FUNC) == nil then
    self:set_field(ENDPOINT_TO_COMPONENT_FUNC, ep_comp_fn)
  else
    log.error("Attempt to re-set valid endpoint-to-component function.")
  end
end
...
function ZwaveDevice:set_update_preferences_fn(update_pref_fn)
  ...
  if self:get_field(constants.UPDATE_PREFERENCES_FUNC) == nil then
    ...
    log.error("Attempt to re-set valid update-preferences function.")
  end
end
...
function ZwaveDevice:send(cmd)
  self.log.info(string.format("sending Z-Wave command: %s", cmd))
...
function ZwaveDevice:send_to_component(cmd, component_id)
  local encap = cmd.encap or zw.ENCAP.AUTO
  local src_channel = cmd.src_channel or 0
  local dst_channels = self:component_to_endpoint(component_id)
  self.log.info(string.format("sending Z-Wave command: %s", cmd))
  self.zwave_channel:send(self.id,
    encap,
    cmd.cmd_class, cmd.cmd_id, cmd.payload,
    src_channel,
    dst_channels)
end
