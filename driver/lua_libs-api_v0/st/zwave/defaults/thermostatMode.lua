...
local function thermostat_supported_modes_report_handler(self, device, cmd)
  ...
  device:emit_event_for_endpoint(cmd.src_channel, capabilities.thermostatMode.supportedThermostatModes(supported_modes))
end
...
local function get_refresh_commands(driver, device)
  if device:supports_capability_by_id(capabilities.thermostatMode.ID) and device:is_cc_supported(cc.THERMOSTAT_MODE) then
    return {ThermostatMode:Get({})}
  end
end
