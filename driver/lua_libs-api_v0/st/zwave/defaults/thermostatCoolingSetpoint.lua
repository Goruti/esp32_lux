...
local function get_refresh_commands(driver, device)
  if device:supports_capability_by_id(capabilities.thermostatCoolingSetpoint.ID) and device:is_cc_supported(cc.THERMOSTAT_SETPOINT) then
    return {ThermostatSetpoint:Get({setpoint_type = ThermostatSetpoint.setpoint_type.COOLING_1})}
  end
end
