...
local function get_refresh_commands(driver, device)
  if device:supports_capability_by_id(capabilities.thermostatOperatingState.ID) and device:is_cc_supported(cc.THERMOSTAT_OPERATING_STATE) then
    return {ThermostatOperatingState:Get({})}
  end
end
