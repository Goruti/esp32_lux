...
local function get_refresh_commands(self, device)
  if device:supports_capability_by_id(capabilities.waterSensor.ID) and device:is_cc_supported(cc.SENSOR_BINARY) then
    return {SensorBinary:Get({sensor_type = SensorBinary.sensor_type.WATER})}
  end
end
