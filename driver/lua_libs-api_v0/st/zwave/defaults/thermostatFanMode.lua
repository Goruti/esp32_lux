...
local function thermostat_supported_fan_modes_report_handler(self, device, cmd)
  ...
  device:emit_event_for_endpoint(cmd.src_channel, capabilities.thermostatFanMode.supportedThermostatFanModes(supported_fan_modes))
end
...
local function get_refresh_commands(driver, device)
  if device:supports_capability_by_id(capabilities.thermostatFanMode.ID) and device:is_cc_supported(cc.THERMOSTAT_FAN_MODE) then
    return {ThermostatFanMode:Get({})}
  end
end
