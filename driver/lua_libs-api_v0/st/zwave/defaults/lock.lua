  ...
  if (cmd.args.notification_type == Notification.notification_type.ACCESS_CONTROL) then
    ...
    if (event ~= nil) then
      ...
      elseif (event_code == access_control_event.KEYPAD_LOCK_OPERATION or event_code == access_control_event.KEYPAD_UNLOCK_OPERATION) then
        if (device:supports_capability(capabilities.lockCodes) and cmd.args.event_parameter ~= nil) then
          local lock_codes = device:get_field(constants.LOCK_CODES)
          local event_params = {cmd.args.event_parameter:byte(1,-1)}
          local code_id = (#event_params == 1) and string.char(event_params[1]) or string.char(event_params[3])
          local code_name = lock_codes[tostring(code_id)] == nil and lock_codes[tostring(code_id)] or "Code " .. code_id
          event["data"] = { codeId = code_id, codeName = code_name, usedCode = tonumber(code_id), method = event["data"].method}
        end
      end
...
local function door_lock_operation_report_handler(driver, device, cmd)
  ...
  -- because some locks report the less-useful door lock operation report before the notification event, the extra
  -- info in the notification event might get ignored as a duplicate event. Because of this, we'll hold on to any
  -- door lock operation report for a few seconds before sending it, to make sure the notification event is the
  -- non-duplicate

  driver:call_with_delay(3, function(d) if (event ~= nil) then device:emit_event(event) end end, "delay message")
end
...
local function get_refresh_commands(self, device)
  if device:supports_capability_by_id(capabilities.lock.ID) and device:is_cc_supported(cc.DOOR_LOCK) then
    return {DoorLock:OperationGet({})}
  end
end

--- @module st.zwave.defaults.lock
--- @alias lock_defaults
