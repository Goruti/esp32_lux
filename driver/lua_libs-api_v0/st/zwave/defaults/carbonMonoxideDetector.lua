...
local function notification_handler(self, device, cmd)
  if cmd.version < 3 then
    ...
    -- subdriver for cc.ALARM ( V1 and V2) should be used
      log.warn("Unhandled Alarm V2 CO command received.")
      return
    end
...
local function get_refresh_commands(self, device)
  if device:supports_capability_by_id(capabilities.carbonMonoxideDetector.ID) and device:is_cc_supported(cc.SENSOR_BINARY) then
    return {SensorBinary:Get({ sensor_type = SensorBinary.sensor_type.CO })}
  end
end

--- @module st.zwave.defaults.carbonMonoxideDetector
