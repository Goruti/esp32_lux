...
capabilities.build_cap_from_json_string = function(cap_json)
  ...
  local cap = capabilities.build_capability(cap_raw_tab.name:gsub(" ", ""), cap_raw_tab.id, cap_raw_tab.version)
...
local cap_not_found_mt = {
  __call = function(proto, version)
    ...
    return capabilities.get_capability_version(proto.ID, version)
  end,
  __index = function(self, key)
    ...
    if raw_val ~= nil then
      ...
      error(string.format("Capability %s version %d definition not found", self.ID, self.version))
    end
  end
}
...
capabilities.raw_event_to_edge_event = function(component_id, event)
...
capability_utils.emit_event = function(device, component_id, sock, event)
  ...
  if is_valid then
    local edge_device_event = capabilities.raw_event_to_edge_event(component_id, event)
    device.log.info(string.format("emitting event: %s", json.encode(edge_device_event)))
    ...
    log.warn("Received invalid value: " .. utils.stringify_table(event.value) .. " for " .. event.capability.NAME .. "." .. event.attribute.NAME)
    return nil
  end
end
