...
mock_device_module.build_test_generic_device = function(device_template, additional_mock_fields)
  ...
  if (device_template.profile.preferences ~= nil) then
    for title, pref in pairs(device_template.profile.preferences) do
      device_template.preferences[utils.camel_case(title)] = pref.definition.default
    end
